...
export default function PersonalInfoSection({ patient }) {
  ...
  const handleSave = async () => {
    if (!formData.gender || !formData.bloodType) {
      setMessage('Please select both gender and blood type.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_ENDPOINTS.UPDATE_PATIENT_PROFILE(patientId)}`, {
        method: 'PUT',
  ...
  return (
    <div style={{ padding: '0 16px' }}>
      ...
      {['name', 'age', 'contactInfo', 'height', 'weight'].map(field => (
